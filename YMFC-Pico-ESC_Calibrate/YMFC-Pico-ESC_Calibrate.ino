...
void setup(){
  ...
  while(eeprom_data[33] != 'H' || eeprom_data[34] != 'D' || eeprom_data[35] != 'R'){
    delay(500);                                                                         //Wait for 500ms.
    digitalWrite(10, !digitalRead(10));                                                 //Change the led status to indicate error.
  }
  ...
  if(data == '1' || data == '2' || data == '3' || data == '4' || data == '5'){          //If motor 1, 2, 3 or 4 is selected by the user.
    ...
    if(new_function_request == false){                                                  //When the throttle was in the lowest position do this.
      ...
      if(eeprom_data[31] == 1){                                                         //The MPU-6050 is installed
        ...
        Wire1.requestFrom(gyro_address,6);                                               //Request 6 bytes from the gyro.
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      for (cal_int = 0; cal_int < 2000 ; cal_int ++){                                   //Take 2000 readings for calibration.
        ...
        gpio_put(2, true);                                                              //Set digital port 2 HIGH.
        gpio_put(3, true);                                                              //Set digital port 3 HIGH.
        gpio_put(4, true);                                                              //Set digital port 4 HIGH.
        gpio_put(5, true);                                                              //Set digital port 5 HIGH.
        delayMicroseconds(1000);                                                        //Wait 1000us.
        gpio_put(2, false);                                                             //Set digital port 2 LOW.
        gpio_put(3, false);                                                             //Set digital port 3 LOW.
        gpio_put(4, false);                                                             //Set digital port 4 LOW.
        gpio_put(5, false);                                                             //Set digital port 5 LOW.
...
void esc_pulse_output(){
  zero_timer = micros();
  gpio_put(2, true);                                                 //Set port 2 to HIGH
  gpio_put(3, true);                                                 //Set port 3 to HIGH
  gpio_put(4, true);                                                 //Set port 4 to HIGH
  gpio_put(5, true);                                                 //Set port 5 to HIGH
...
void set_gyro_registers(){
  //Setup the MPU-6050
  if(eeprom_data[31] == 1){
    Wire1.beginTransmission(gyro_address);                        //Start communication with the address found during search.
...
void gyro_signalen(){
  ...
  if(eeprom_data[31] == 1){
    ...
    Wire1.write(0x3B);                                            //Start reading @ register 43h and auto increment with every read.
    Wire1.endTransmission();                                      //End the transmission.
    Wire1.requestFrom(gyro_address,14);                           //Request 14 bytes from the gyro.
    while(Wire1.available() < 14);                                //Wait until the 14 bytes are received.
    acc_axis[1] = Wire1.read()<<8|Wire1.read();                    //Add the low and high byte to the acc_x variable.
    acc_axis[2] = Wire1.read()<<8|Wire1.read();                    //Add the low and high byte to the acc_y variable.
    acc_axis[3] = Wire1.read()<<8|Wire1.read();                    //Add the low and high byte to the acc_z variable.
    temperature = Wire1.read()<<8|Wire1.read();                    //Add the low and high byte to the temperature variable.
    gyro_axis[1] = Wire1.read()<<8|Wire1.read();                   //Read high and low part of the angular data.
    gyro_axis[2] = Wire1.read()<<8|Wire1.read();                   //Read high and low part of the angular data.
    gyro_axis[3] = Wire1.read()<<8|Wire1.read();                   //Read high and low part of the angular data.
  }
