...
// Import libraries required for communicating with the gyro and storing important data
#include <Wire.h>
#include <EEPROM.h>
...
void setup(){
  ...
  gpio_set_irq_enabled_with_callback(7, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, &gpio_callback); // Set GP7 to trigger an interrupt on state change
  gpio_set_irq_enabled_with_callback(8, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, &gpio_callback); // Set GP8 to trigger an interrupt on state change
  gpio_set_irq_enabled_with_callback(9, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, &gpio_callback); // Set GP9 to trigger an interrupt on state change
  ...
  EEPROM.begin(512);        // Initialize 512 bytes of virtual EEPROM
...
void loop(){
  ...
  if(error == 0){
    ...
    delay(3000);
    Serial.println(F("Calibrating the gyro, this will take about 8 seconds"));
  ...
  if(error == 0){
    ...
    Serial.println(F("The LED should now be lit red"));
    Serial.println(F("Move stick 'nose up' and back to center to continue"));
    check_to_continue();
    digitalWrite(10, LOW);
  }
  ...
  if(error == 0){
    ...
    Serial.println(F("Verifying EEPROM data"));
    delay(1000);
    ...
    if('H' != EEPROM.read(33))error = 1;
    if('D' != EEPROM.read(34))error = 1;
    if('R' != EEPROM.read(35))error = 1;
  ...
  if(error == 0){
    Serial.println(F("Setup is finished. Check if the LED is blinking to confirm."));
...
byte search_gyro(int gyro_address, int who_am_i){
  ...
  Wire1.requestFrom(gyro_address, 1);
  timer = millis() + 100;
  while(Wire1.available() < 1 && timer > millis());
  lowByte = Wire1.read();
...
void start_gyro(){
  ...
  if(type == 2 || type == 3){
    Wire1.beginTransmission(address);                             //Start communication with the gyro with the address found during search
    Wire1.write(0x20);                                            //We want to write to register 1 (20 hex)
    Wire1.write(0x0F);                                            //Set the register bits as 00001111 (Turn on the gyro and enable all axis)
    Wire1.endTransmission();                                      //End the transmission with the gyro

    Wire1.beginTransmission(address);                             //Start communication with the gyro (adress 1101001)
    Wire1.write(0x20);                                            //Start reading @ register 28h and auto increment with every read
    Wire1.endTransmission();                                      //End the transmission
    Wire1.requestFrom(address, 1);                                //Request 6 bytes from the gyro
    while(Wire1.available() < 1);                                 //Wait until the 1 byte is received
    Serial.print(F("Register 0x20 is set to:"));
    Serial.println(Wire1.read(),BIN);

    Wire1.beginTransmission(address);                             //Start communication with the gyro  with the address found during search
    Wire1.write(0x23);                                            //We want to write to register 4 (23 hex)
    Wire1.write(0x90);                                            //Set the register bits as 10010000 (Block Data Update active & 500dps full scale)
    Wire1.endTransmission();                                      //End the transmission with the gyro
    
    Wire1.beginTransmission(address);                             //Start communication with the gyro (adress 1101001)
    Wire1.write(0x23);                                            //Start reading @ register 28h and auto increment with every read
    Wire1.endTransmission();                                      //End the transmission
    Wire1.requestFrom(address, 1);                                //Request 6 bytes from the gyro
    while(Wire1.available() < 1);                                 //Wait until the 1 byte is received
  ...
  if(type == 1){
    ...
    Wire1.beginTransmission(address);                             //Start communication with the gyro
    Wire1.write(0x6B);                                            //PWR_MGMT_1 register
    Wire1.write(0x00);                                            //Set to zero to turn on the gyro
    Wire1.endTransmission();                                      //End the transmission
    
    Wire1.beginTransmission(address);                             //Start communication with the gyro
    Wire1.write(0x6B);                                            //Start reading @ register 28h and auto increment with every read
    Wire1.endTransmission();                                      //End the transmission
    Wire1.requestFrom(address, 1);                                //Request 1 bytes from the gyro
    while(Wire1.available() < 1);                                 //Wait until the 1 byte is received
    ...
    Wire1.beginTransmission(address);                             //Start communication with the gyro (adress 1101001)
    Wire1.write(0x1B);                                            //Start reading @ register 28h and auto increment with every read
    Wire1.endTransmission();                                      //End the transmission
    Wire1.requestFrom(address, 1);                                //Request 1 bytes from the gyro
    while(Wire1.available() < 1);                                 //Wait until the 1 byte is received
...
void gyro_signalen(){
  if(type == 2 || type == 3){
    Wire1.beginTransmission(address);                             //Start communication with the gyro
    Wire1.write(168);                                             //Start reading @ register 28h and auto increment with every read
    Wire1.endTransmission();                                      //End the transmission
    Wire1.requestFrom(address, 6);                                //Request 6 bytes from the gyro
    while(Wire1.available() < 6);                                 //Wait until the 6 bytes are received
    lowByte = Wire1.read();                                       //First received byte is the low part of the angular data
    highByte = Wire1.read();                                      //Second received byte is the high part of the angular data
  ...
  if(type == 1){
    Wire1.beginTransmission(address);                             //Start communication with the gyro
    Wire1.write(0x43);                                            //Start reading @ register 43h and auto increment with every read
    Wire1.endTransmission();                                      //End the transmission
    Wire1.requestFrom(address,6);                                 //Request 6 bytes from the gyro
    while(Wire1.available() < 6);                                 //Wait until the 6 bytes are received
...
//This routine is called every time input 6, 7, 8 or 9 changed state
//ISR(PCINT0_vect){
...
void intro(){
  ...
  delay(500);
  Serial.println(F("        For RPI Pico"));
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("YMFC-Pico Setup Program"));
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  delay(1500);
  Serial.println(F("For support and questions, contact no one :("));
  Serial.println(F(""));
