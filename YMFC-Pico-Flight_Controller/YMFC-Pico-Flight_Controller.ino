...
void setup(){
  ...
  while(eeprom_data[33] != 'H' || eeprom_data[34] != 'D' || eeprom_data[35] != 'R')delay(10);
  ...
  while(eeprom_data[31] == 2 || eeprom_data[31] == 3)delay(10);

  set_gyro_registers();                                                     //Set the specific gyro registers.

  for (cal_int = 0; cal_int < 1250 ; cal_int ++){                           //Wait 5 seconds before continuing.
    //We don't want the esc's to be beeping annoyingly. So let's give them a 1000us pulse while reading the gyro.
  ...
  for (cal_int = 0; cal_int < 2000 ; cal_int ++){                           //Take 2000 readings for calibration.
    if(cal_int % 15 == 0)digitalWrite(10, !digitalRead(10));                //Change the led status to indicate calibration.
  ...
  // Set GP6-GP9 to trigger an interrupt on state change
  gpio_set_irq_enabled_with_callback(6, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, &gpio_callback);
...
void loop(){
  ...
  if(battery_voltage < 1000 && battery_voltage > 600)digitalWrite(10, HIGH);
  ...
  if(micros() - loop_timer > 4050)digitalWrite(10, HIGH);                   //Turn on the LED if the loop time exceeds 4050us.
